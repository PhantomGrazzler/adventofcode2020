...
const std::string input = R"(4-6 b: bbbdbtbbbj
1-6 g: ggvggbgggstg
1-4 s: lssss
...
7-8 v: tvvvvvvhv
3-10 d: bxktdrtddtdtsh)" + std::string( R"(
4-12 j: npwxjjjjbjkq
1-4 l: tlllll
...
{
    PasswordDetails( const PasswordPolicy& pp, const std::string& pw )
        : policy( pp )
        , password( pw )
    {}
...
bool PasswordValid( const PasswordPolicy& policy, const std::string& password )
{
    const auto charCount = std::count( password.cbegin(), password.cend(), policy.character );
    return charCount >= policy.min && charCount <= policy.max;
}

int main()
{
    std::vector<PasswordDetails> pwDetails;
    pwDetails.reserve( 1024 );
    std::regex re( R"((\d+)-(\d+) (\w): (\w+))" );
    std::smatch match;
    ...
    while ( std::getline( iss, line ) )
    {
        if ( std::regex_match( line, match, re ) )
        {
            const PasswordPolicy policy{
                ...
                match[3].str()[0] };
            pwDetails.emplace_back( policy, match[4].str() );
        }
        else
        {
            std::cerr << "No match for '" << line << "'\n";
        }
    }

    const auto validPasswordCountPart1 =
        std::count_if( pwDetails.cbegin(), pwDetails.cend(), []( const PasswordDetails& details ) {
            const auto charCount = std::count(
                details.password.cbegin(),
                details.password.cend(),
                details.policy.character );
            return charCount >= details.policy.min && charCount <= details.policy.max;
        } );

    std::cout << "Valid passwords part 1: " << validPasswordCountPart1 << '\n';

    const auto validPasswordCountPart2 =
        std::count_if( pwDetails.cbegin(), pwDetails.cend(), []( const PasswordDetails& details ) {
            const bool firstMatches =
                details.password.at( details.policy.min - 1 ) == details.policy.character;
            const bool secondMatches =
                details.password.at( details.policy.max - 1 ) == details.policy.character;
            return ( firstMatches && !secondMatches ) || ( !firstMatches && secondMatches );
        } );

    std::cout << "Valid passwords part 2: " << validPasswordCountPart2 << '\n';
