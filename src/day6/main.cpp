...
const std::string sampleInput = R"(abc
...
{
private:
    std::vector<std::set<char>> m_answers;

public:
    void add_answers( const std::string& answersString )
    {
        std::set<char> answers;
        std::for_each( answersString.cbegin(), answersString.cend(), [&answers]( const char c ) {
            answers.insert( c );
        } );
        m_answers.push_back( std::move( answers ) );
    }
    ...
    std::size_t intersection_count() const
    {
        if ( m_answers.size() == 1 )
        {
            return m_answers.back().size();
        }
        else
        {
            std::set<char> commonAnswers;
            std::set_intersection(
                m_answers.at( 0 ).cbegin(),
                m_answers.at( 0 ).cend(),
                m_answers.at( 1 ).cbegin(),
                m_answers.at( 1 ).cend(),
                std::inserter( commonAnswers, commonAnswers.end() ) );

            for ( std::size_t i = 2; i < m_answers.size(); i++ )
            {
                std::set<char> newCommonAnswers;
                std::set_intersection(
                    commonAnswers.cbegin(),
                    commonAnswers.cend(),
                    m_answers.at( i ).cbegin(),
                    m_answers.at( i ).cend(),
                    std::inserter( newCommonAnswers, newCommonAnswers.end() ) );
...
int main()
{
...

    {
        ...
        for ( const auto& line : lines )
        {
            if ( !line.empty() )
            {
                std::for_each( line.cbegin(), line.cend(), [&groupAnswers]( const char c ) {
    ...
    {
        std::vector<GroupAnswers> groupAnswerCollection;
        ...
        for ( const auto& line : lines )
        {
            if ( !line.empty() )
            {
                groupAnswers.add_answers( line );
            }
            else
            {
                groupAnswerCollection.push_back( std::move( groupAnswers ) );
                groupAnswers.clear();
            }
        }

        const std::size_t intersectionCount = std::transform_reduce(
            groupAnswerCollection.cbegin(),
            groupAnswerCollection.cend(),
            std::size_t{ 0 },
            std::plus(),
            []( const auto& ga ) { return ga.intersection_count(); } );
